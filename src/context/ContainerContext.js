...
export const ContainerProvider = ({ children }) => {
  ...
  const fetchContainers = async (status = 'all') => {
    try {
      const response = await axios.get(`http://192.168.100.148:3230/api/container/fetch?status=${status}`);
  ...
  const updateContainerState = async (containerId, command) => {
    try {
      await axios.get(`http://192.168.100.148:3230/api/container/command?container=${containerId}&command=${command}`);
      const response = await axios.get(`http://192.168.100.148:3230/api/container/fetchById?container=${containerId}`);
